...
Los errores de coordenadas o centros ausentes deben notificarse al organismo y mediante la plantilla correspondiente. No edites datos oficiales para ocultarlos. Un override temporal JSON en `config/overrides/` debe declarar `center_code`, `field`, `old_value`, `new_value`, `reason`, `source`, `created_at` y `expires_at`; está desactivado por defecto, se registra en el manifiesto y se retira al corregirse la fuente.
